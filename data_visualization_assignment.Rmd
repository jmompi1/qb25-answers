...
#The distribution is highly skewed left. There is a high proportion of genes that are underexpressed. This also seems to be the case for other tissue types. Most genes overall are not highly represented regardless of the tissue type. 

ggplot(dt, aes(x = Testis))+
  geom_histogram(breaks = seq(0,100,2))

ggplot(dt, aes(x = `Heart - Atrial Appendage`))+
  geom_histogram(breaks = seq(0,100,2))
...
ggplot(dt_modified, aes(x = Liver+1, y = `Heart - Left Ventricle`+1, color = is_mitochondrial)) +
geom_point(size = 1, alpha = 0.2)+
  scale_x_log10() +
  scale_y_log10() +
  geom_abline(slope = 1, lty = "dashed", color = "red")
